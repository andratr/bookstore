...
function IssueForm({
  ...
  const validate = async () => {
    try {
      ...
      const response = await GetUserById(readerId);
      if (response.success) {
        if (response.data.role !== "reader") {
          setValidated(false);
          setErrorMessage("This user is not a reader");
      ...
      <div className="flex flex-col gap-2">
        ...
        {validated && (
          <div className="bg-secondary p-1 text-white">
            <h1 className="text-sm">reader : {readerData.name}</h1>
            <h1>
